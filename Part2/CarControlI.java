...
interface CarControlI {
     ...
     * but not with itself.
     *
     * barrierSet(k) should set the barrier threshold to k. However, if the
     * threshold is
     * raised while cars are waiting, the threshold must not be changed until the
     * next release
     * of the barrier (using the current threshold). In this case, the call must
     * block until then.
     */
