...
interface CarI {
    ...
    // Adds pos as an extension to the current route (potentially defining a new
    // one).
    // Returns a unique id for this destination.
    public long addDestination(Pos pos);

    // As above. In addition, the run() method of the callback is executed when the
    // destination
    // is reached. Callbacks are called sequentially in arrival order by an external
    // thread.
